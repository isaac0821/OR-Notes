	...
	\chapter{Minimum Weight Flow Problem}
		...
		\section{Network Simplex Method}
			...
			\begin{figure}[!ht]
				...
				\begin{tikzpicture}[node distance=2.5cm]
					...
					\draw [arrow, dashed] (1) to [out = -50, in = 110] node [left] {j (to enter)} (3);
				\end{tikzpicture}
			\end{figure}
		...
		\section{Out-of-Kilter algorithm}
			...
			\begin{figure}
				\centering
				\begin{tikzpicture}
					\node (2) [circleNode] {2, -2};
					\node (1) [circleNode, below of=2] {1, -1};
					\node (3) [circleNode, right of=2] {3, 2};
					\node (4) [circleNode, below of=3] {4, 1};
					\draw (2) -- node [above] {5} (3);
					\draw (2) -- node [above] {11} (4);
					\draw (1) -- node [above] {7} (3);
					\draw (1) -- node [above] {12} (4);
				\end{tikzpicture}
			\end{figure}
			...
			\begin{figure}
				...
				\begin{tikzpicture} [node distance = 2cm]
					\node (2) [circleNode] {2, -2};
					\node (s) [circleNode, below of=2, xshift = -1cm] {s};
					\node (1) [circleNode, below of=s, xshift = 1cm] {1, -1};
					\node (3) [circleNode, right of=2] {3, 2};
					\node (t) [circleNode, below of=3, xshift = 1cm] {t};
					\node (4) [circleNode, below of=t, xshift = -1cm] {4, 1};
					\draw (s) -- node [above] {0, 2, 0} (2);
					\draw (s) -- node [above] {0, 1, 0} (1);
					\draw (2) -- node [above] {0, 3, 5} (3);
					\draw (2) -- node [above] {0, 3, 11} (4);
					\draw (1) -- node [above] {0, 3, 7} (3);
					\draw (1) -- node [above] {0, 3, 12} (4);
					\draw (3) -- node [above] {0, 2, 0} (t);
					\draw (4) -- node [above] {0, 3, 0} (t);
					\draw (t) -- node [below] {3, 3, 0} (s);
				\end{tikzpicture}
			\end{figure}

			\begin{theorem}
				(For LP optimality conditions we need primal feasibility, dual feasibility and complementary slackness. Primal and dual feasibility are obvious so we need to show complementary slackness through following theorem.)
			...
			\begin{proof}
				For each $e \in A$ define $\gamma_e = \max\{y_{h(e)} - y_{t(e)} - w_e, 0\}$, $\mu_e = \max\{w_e - y_{h(e)} + y_{t(e)}, 0\}$, $\gamma_e - \mu_e = y_{h(e)} - y_{t(e)} - w_e$.
				\begin{align}
					\sum_{e\in A} (\mu_el_e - \gamma_eu_e) &= \sum_{e\in A} (\mu_el_e - \gamma_eu_e) + \sum_{i \in V}y_i(\sum_{h(e) = i} x_e - \sum_{t(e) = i} x_e)\\

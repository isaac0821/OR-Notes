...
\begin{document}
	...
	% Levels of Contents 
	% \part{[Topic enough for one semester]}
	% 	\chapter{[Topic worth for a few weeks lessons]}
	% 		\section{[Topic that discussing it could be one 30 pages article]}
	% 			\subsection{[One algorithm, one theory, one theorem with proof, or a set of related definitions]}
	...
	\part{Preliminary Topics}\label{PT}
		...
		\chapter{Basic Concepts of Computational and Complexity Theory}
			...
			\section{NP and Computational Intractability}
				\subsection{P, NP and Co-NP}
					...
					\notice{If the instance is ``easy'' to prove to be true, then it is in NP, if the counter instance is ``easy'' to prove to be false, then it is in Co-NP}
				...
				\subsection{NP-Completeness}
					...
					\begin{proof}
						The following process $Clear(S, x)$ can be executed within $O(n)$, in which $S$ is a CNF with at least one clause being singleton $x$ (or $\neg x$), where $n$ is the length of CNF $S$, if it returns $S^\prime$, $S^\prime$ will be a CNF without $x$ (or $\neg x$) in any of clause in $S$. Otherwise, if it returns null, then $S$ is not satisfiable.
						\begin{algorithm}
							\caption{Clear(S, x)}
							\begin{algorithmic}[1]
								\State Initialize $S^\prime \gets S$
								\For {Each clause $c$ in $S^\prime$}
									\If {$c$ is a singleton $x$}
										\State Remove singleton clause $x$ in $S^\prime$
									\ElsIf {$c$ is a singleton $\neg x$}
										\State \Return null
									\ElsIf {$c$ is a 2-clause with $x$}
										\State Remove singleton clause $c$ in $S^\prime$
									\ElsIf {$c$ is a 2-clause with $\neg x$}
										\State Remove $\neg x$ in clause $c$ in $S^\prime$
									\EndIf
								\EndFor
						...
						\begin{algorithm}
							\caption{2-SAT(S)}
							\begin{algorithmic}[1]
								\While {Length of $S$ is greater than 0}
									\If {$S$ has a singleton $x$ (or $\neg x$)}
										\State $S \gets$ Clear($S, x$) (or $S \gets$ Clear($S, \neg x$))
										\If {$S == null$}
											\State \Return False
										\EndIf
									\Else
										\State Denote $S = (x_1 \wedge x_2) \vee S_r$
										\State $S_1 \gets x_1 \vee S_r$
										\State $S_2 \gets x_2 \vee S_r$
										\If {2-SAT($S_1$)}
											\State \Return True
										\ElsIf{2-SAT($S_2$)}
											\State \Return True
	...
	\part{Stochastic Methods}\label{Sto}
		\chapter{Markov Chain}

		\chapter{Queueing Theory}

	\part{Inventory Theory}\label{Inv}

...
\begin{document}
	...
	\part{Linear Programming}
		...
		\chapter{Polynomial-Time Algorithm for Linear Programming}

	\part{Integer and Combinatorial Programming}
		\chapter{Polyhedral Theory}
	...
	\part{Heuristic Optimization}
		\chapter{}

	\part*{Special Topic: Vehicle Routing Problem}
		...
		\chapter{Traveling Salesman Problem}
			...
			\section{Up and Lower Bounds}

		\chapter{Vehicle Routing Problem}
			The Vehicle Routing Problem (VRP) is defined on a graph $G=(V, E)$ where $V = \{0, \dots, n\}$, is a vertex set. $E = \{(i, j) | i, j \in V\}$ represents an edge set. Usually, vertex $0$ is a depot while the remaining vertices are customers, for simplicity, researchers sometimes add a dummy vertex $N^\prime$ as a replicate of depot, so that vehicles leave depot from vertex $0$ and return to depot at vertex $N^\prime$, the distance or cost between vertex $0$ and vertex $N^\prime$ is 0. 

		\chapter{Important Variants}
			\section{VRPTW}
		...
		\chapter{Heuristic Algorithms}
			\section{Clarke and Wright Saving Algorithm}

			\section{Sweep Algorithm}

			\section{Mole and Jameson Insertion Algorithm}

			\section{Christofides Insertion Algorithm}

			\section{Petal Algorithms}

			\section{Cluster-first, Route-second Algorithm}

			\section{Lin's \texorpdfstring{$\lambda$}{Lg}-opt Local Improvement}
